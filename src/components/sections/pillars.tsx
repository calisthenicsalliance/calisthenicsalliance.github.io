...
export function Pillars() {
	...
	return (
		<Section>
			...
			<Reveal className="mt-14 grid gap-px overflow-hidden rounded-md border border-white/10 bg-white/10 sm:grid-cols-3">
				{items.map((item, index) => {
					const Icon = icons[index] ?? Trophy;
					return (
						<div key={item.title} className="group relative flex flex-col gap-4 bg-background p-8">
