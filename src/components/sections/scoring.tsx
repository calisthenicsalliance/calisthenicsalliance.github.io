	...
	return (
		<Section tone="raised">
			...
			<div className="mt-14 grid gap-6 lg:grid-cols-[1.2fr_1fr]">
				<Reveal className="grid grid-cols-2 gap-px overflow-hidden rounded-md border border-white/10 bg-white/10 sm:grid-cols-4">
					{battleResults.map((result) => (
						<div
							key={result.key}
							className="flex h-full flex-col items-center justify-center gap-2 bg-background p-8 text-center">
							<span
								className={
									result.key === "win"
